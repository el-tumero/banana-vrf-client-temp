...
import (
	...
	"github.com/el-tumero/banana-vrf-client-temp/proposals"
	"github.com/el-tumero/banana-vrf-client-temp/user"
...
func DecisionProc(ctx context.Context, u *user.User, conn *websocket.Conn, clsChan chan struct{}) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-clsChan:
			fmt.Println("DecProc closed - client disconnected")
		...
		case header := <-headers:
			...
			if currentRoundId == 0 {
				...
				if err != nil {
					fmt.Println("can't get current round id ", err)
					break
				}
			...
			if err != nil {
				fmt.Println("can't get round data ", err)
				break
			}
			...
			if r.State == ROUND_EMPTY && block.Number().Cmp(GetDecisionHeight(r.BlockHeight)) == 1 {
				...
				if smallest == nil {
					fmt.Println("no values in storage...")
					break
				}
